...
def build_series(upper_bound: int):
    NUM_OF_DIGITS = 10**10
    residue_sequence = [
        pwr_residue(n, n, NUM_OF_DIGITS) for n in range(1, upper_bound + 1)
    ]
    residue_series = [1] * upper_bound
    for i in range(1, upper_bound):
        residue_series[i] = (
            residue_series[i - 1] + residue_sequence[i]
        ) - NUM_OF_DIGITS * (
            (residue_series[i - 1] + residue_sequence[i]) >= NUM_OF_DIGITS
        )
    if DEBUG:
        print(
            f"The lengths of the lists are {len(residue_sequence), len(residue_series)}"
        )
